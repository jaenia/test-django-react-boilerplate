...
- Create a copy of ``todolist2/settings/local.py.example``:  
  `cp todolist2/settings/local.py.example todolist2/settings/local.py`
...
- Deregister the example app by removing `'exampleapp.apps.ExampleappConfig'` from ``backend/todolist2/settings/base.py``.
- Adjust ``backend/todolist2/urls.py`` to point to your newly created Django app and remove the path configuration that redirects to the deleted example app.
